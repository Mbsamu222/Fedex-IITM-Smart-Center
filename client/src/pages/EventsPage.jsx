            ...
            <div className="flex flex-wrap gap-2">
              {filterTabs.map(tab => (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-sm 2xl:text-base font-medium transition-colors ${
                    activeFilter === tab
                ...
                <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {paginatedEvents.map((event, idx) => (
                    <Link key={event.id || idx} to={`/events/${event.slug || event.id}`} className="group flex flex-col rounded-3xl border border-border bg-card overflow-hidden transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-[var(--shadow-lift)]">
                      <div className="w-full aspect-[16/9] shrink-0 overflow-hidden bg-gradient-to-br from-purple-100 via-indigo-50 to-orange-100 relative flex items-center justify-center">
                        {event.image_url ? (
                          <img 
                          ...
                          <div className="flex items-center justify-center text-primary/40 font-display font-bold text-lg">
                            IITM SMART
                          </div>
                        )}
